...
def obtener_textos_dentro_de_polilinea(polilinea, textos):
    """
    Función optimizada que encuentra todos los textos y bloques dentro de una polilínea,
    priorizando los que están completamente dentro antes de buscar en los alrededores.
    Extrae y retorna toda la información de los bloques.
    """
    # Convertir los puntos de la polilínea a coordenadas para Shapely
    vertices = [(p[0], p[1]) for p in polilinea]
    poligono = Polygon(vertices)
    centro_polilinea = poligono.centroid
    
    # Lista para resultados y conjunto para evitar duplicados
    textos_en_polilinea = []
    textos_encontrados = set()
    
    # Crear un buffer para capturar elementos en el borde
    poligono_con_buffer = poligono.buffer(0.5)  # Buffer moderado
    
    def validar_formato_texto(texto):
        """Función para validar y formatear el texto encontrado."""
        try:
            ...
            print(f"Error al validar formato: {e}")
            return texto  # Devolver original si hay error
    
    # FASE 1: Buscar textos dentro de la polilínea
    print("\n=== BUSCANDO TEXTOS ===")
    for text in textos:
        if text.dxftype() in ['TEXT', 'MTEXT']:
            ...
            if poligono.contains(punto_texto):
                ...
                if texto_formateado.strip() and texto_formateado not in textos_encontrados:
                    textos_en_polilinea.append(texto_formateado)
                    textos_encontrados.add(texto_formateado)
                    print(f"✓ Texto encontrado dentro: {texto_formateado}")
    
    # FASE 2: Buscar BLOQUES DENTRO de la polilínea (prioridad máxima)
    print("\n=== BUSCANDO BLOQUES DENTRO ===")
    for entity in textos:
        if entity.dxftype() == 'INSERT':
            punto_bloque = Point(entity.dxf.insert)
            
            # Verificar si está DENTRO
            if poligono.contains(punto_bloque):
                print(f"✓ BLOQUE INTERNO encontrado: {entity.dxf.name} en capa {entity.dxf.layer}")
                    ...
                    if hasattr(entity, 'attribs'):
                        for attrib in entity.attribs:
                            if hasattr(attrib.dxf, 'text'):
                                texto = attrib.dxf.text
                                tag = attrib.dxf.tag if hasattr(attrib.dxf, 'tag') else "sin_tag"
                                atributos_extraidos.append(texto)
                                print(f"  → Atributo [{tag}]: '{texto}'")
                except Exception as e:
                    print(f"  ✗ Error atributos directos: {e}")
                
                # Método 2: Atributos como hijos
                try:
                    for child in entity:
                        if child.dxftype() == 'ATTRIB' and hasattr(child.dxf, 'text'):
                            texto = child.dxf.text
                            tag = child.dxf.tag if hasattr(child.dxf, 'tag') else "sin_tag"
                            atributos_extraidos.append(texto)
                            print(f"  → Atributo hijo [{tag}]: '{texto}'")
                except Exception as e:
                    print(f"  ✗ Error atributos hijos: {e}")
                
                # Método 3: get_attribs
                try:
                    if hasattr(entity, 'get_attribs'):
                        for attrib in entity.get_attribs():
                            if hasattr(attrib.dxf, 'text'):
                                ...
                                tag = attrib.dxf.tag if hasattr(attrib.dxf, 'tag') else "sin_tag"
                                atributos_extraidos.append(texto)
                                print(f"  → Atributo get_attribs [{tag}]: '{texto}'")
                except Exception as e:
                    print(f"  ✗ Error get_attribs: {e}")
                
                # Agregar atributos encontrados
                for texto in atributos_extraidos:
                    texto_formateado = validar_formato_texto(texto)
                    if texto_formateado.strip() and texto_formateado not in textos_encontrados:
                        textos_en_polilinea.append(texto_formateado)
                        textos_encontrados.add(texto_formateado)
                        print(f"  ✓ Agregado: '{texto_formateado}'")
    
    # FASE 3: Buscar BLOQUES CERCANOS si aún no tenemos resultados
    if len(textos_en_polilinea) < 1:
        print("\n=== BUSCANDO BLOQUES CERCANOS ===")
        for entity in textos:
            if entity.dxftype() == 'INSERT':
                punto_bloque = Point(entity.dxf.insert)
                
                # Solo considerar bloques dentro del buffer pequeño
                if poligono_con_buffer.contains(punto_bloque) and not poligono.contains(punto_bloque):
                    print(f"✓ BLOQUE CERCANO encontrado: {entity.dxf.name} en capa {entity.dxf.layer}")
    ...
    # Resumen final
    print("\n=== RESULTADOS FINALES ===")
    print(f"Total encontrado: {len(textos_en_polilinea)}")
    for i, texto in enumerate(textos_en_polilinea):
        print(f"{i+1}. {texto}")
...
# Print all layer names in the DXF file
        ...
        def procesar_prelosa(polilinea, tipo_prelosa, idx):
            global resultados_prelosas
            if 'resultados_prelosas' not in globals():
                resultados_prelosas = {}
...
#
                ...
                if len(textos_longitudinal) > 0:
                    ...
                    if len(textos_longitudinal) >= 1:
                        ...
                        try:
                            ...
                            print(f"Error al procesar primer texto horizontal en PRELOSA MACIZA 15 '{texto}': {e}")
                    ...
                    if len(textos_longitudinal) >= 2:
                        ...
                        try:
                            ...
                            print(f"Error al procesar segundo texto horizontal en PRELOSA MACIZA 15 '{texto}': {e}")
                
                # Procesar textos verticales
                if len(textos_transversal) > 0:
                    print(f"Procesando {len(textos_transversal)} textos verticales en PRELOSA MACIZA 15")
                    ...
                    if len(textos_transversal) >= 1:
                        ...
                        try:
                            ...
                            print(f"Error al procesar primer texto vertical en PRELOSA MACIZA 15 '{texto}': {e}")
                    ...
                    if len(textos_transversal) >= 2:
                        ...
                        try:
                            ...
                            print(f"Error al procesar segundo texto vertical en PRELOSA MACIZA 15 '{texto}': {e}")
                ...
                if len(textos_long_adi) > 0:
                    ...
                    if len(textos_tra_adi) > 0:
                        print("\n" + "=" * 60)
                        print(f"PROCESANDO {len(textos_tra_adi)} TEXTOS TRANSVERSALES ADI EN PRELOSA MACIZA 15 ESPECIAL")
                        print("=" * 60)
                    ...
                    # Formatear valores para el bloque
                    k8_formateado = formatear_valor_espaciamiento(k8_valor)
                    # Diferencia con PRELOSA MACIZA normal: usamos 10mm para longitudinal en lugar de 3/8"
                    as_long_texto = f"1Ø10mm@.{k8_formateado}"
                    # Diferencia con PRELOSA MACIZA normal: usamos 8mm@.25 para transversal en lugar de 6mm@.28
                    as_tra1_texto = "1Ø8mm@.25"  # Valor fijo para PRELOSA MACIZA 15
                    
                    # NUEVO: Formatear AS_TRA2 usando el valor de K18 si hay textos_tra_adi
                    if len(textos_tra_adi) > 0 and k18_valor is not None:
                        k18_formateado = formatear_valor_espaciamiento(k18_valor)
                        as_tra2_texto = f"1Ø8mm@.{k18_formateado}"
                        print(f"  ★ Valor para AS_TRA2 calculado de K18: {as_tra2_texto}")
                    else:
                        as_tra2_texto = None
                    ...
                    print("\nVALORES FINALES FORMATEADOS PARA BLOQUE:")
                    print(f"  ★ AS_LONG: {as_long_texto} (de K8={k8_valor} -> formato .{k8_formateado})")
                    print(f"  ★ AS_TRA1: {as_tra1_texto} (valor fijo para PRELOSA MACIZA 15)")
                    if as_tra2_texto:
                        print(f"  ★ AS_TRA2: {as_tra2_texto} (de K18={k18_valor} -> formato .{formatear_valor_espaciamiento(k18_valor)})")
                    print("=" * 60)
#
                if len(textos_longitudinal) == 0 and len(textos_transversal) == 0 and len(textos_long_adi) == 0 and len(textos_tra_adi) == 0:
                    print("=" * 60)
                    print("PRELOSA MACIZA 15 SIN NINGÚN TIPO DE ACERO DETECTADO")
                    print("=" * 60)
                    ...
                    print("\nCOLOCANDO VALORES POR DEFECTO EN EXCEL:")
                    print(f"  - Celda G4 = 1")
                    # Diferencia con PRELOSA MACIZA normal: usamos 10mm en lugar de 3/8"
                    print(f"  - Celda H4 = 10mm")
                    print(f"  - Celda J4 = {espaciamiento_macizas_adi}")
                    
                    # Colocar valores por defecto en Excel
                    ws.range('G4').value = 1
                    ws.range('H4').value = "10mm"
                    ws.range('J4').value = espaciamiento_macizas_adi
                    ...
                    # Formatear para bloque (diferente de PRELOSA MACIZA normal)
                    # Usamos 10mm para longitudinal en lugar de 3/8"
                    as_long_texto = f"1Ø10mm@.{formatear_valor_espaciamiento(k8_valor)}"
                    # Usamos 8mm@.25 para transversal en lugar de 6mm@.28
                    as_tra1_texto = "1Ø8mm@.25"  # Valor fijo para PRELOSA MACIZA 15
                    as_tra2_texto = None
                ...
                # Guardar los resultados para insertar posteriormente
                # Usamos diferentes valores para AS_LONG y AS_TRA1 en comparación con PRELOSA MACIZA normal
                if 'as_long_texto' in locals():
                    resultado_prelosa = {
                        'AS_LONG': as_long_texto,
                        'AS_TRA1': as_tra1_texto
                    }
                    if as_tra2_texto:
                        resultado_prelosa['AS_TRA2'] = as_tra2_texto
                    
                    # Guardar el resultado para usar más tarde
                    resultados_prelosas[idx] = resultado_prelosa
                else:
                    print(f"No se generó ningún resultado para la PRELOSA MACIZA 15 #{idx}")
    ...
    # Verificar si tenemos aceros adicionales o valores calculados manualmente
        ...
        for idx, polilineas_macizas_15 in enumerate(polilineas_macizas_15):
            procesar_prelosa(polilineas_macizas_15, "PRELOSA MACIZA 15", idx)
